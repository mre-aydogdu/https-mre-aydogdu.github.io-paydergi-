(function(){const doc=document.documentElement;const parentURL='https://example.com/';function createEl(element='div'){return document.createElement(element);}
...
if(id){link.href=`${current}#${id}`;node.appendChild(link);pushClass(node,'link_owner');}});function copyToClipboard(str){const el=createEl('textarea');el.value=str;el.setAttribute('readonly','');el.style.position='absolute';el.style.left='-9999px';document.body.appendChild(el);const selected=document.getSelection().rangeCount>0?document.getSelection().getRangeAt(0):false;el.select();document.execCommand('copy');document.body.removeChild(el);if(selected){document.getSelection().removeAllRanges();document.getSelection().addRange(selected);}};(function copyHeadingLink(){let deeplink='link';let deeplinks=document.querySelectorAll(`.${deeplink}`);if(deeplinks){document.body.addEventListener('click',function(event)
...
doc.addEventListener('click',function(event){
    ...
    if(isModeToggle){setUserColorMode(true);
    }});})();